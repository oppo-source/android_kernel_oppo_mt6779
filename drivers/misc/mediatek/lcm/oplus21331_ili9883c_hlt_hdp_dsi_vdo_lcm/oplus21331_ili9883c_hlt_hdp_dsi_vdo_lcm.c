...
static void lcm_get_params(struct LCM_PARAMS *params)
{
	...
	register_device_proc("lcd", "ili9883c", "21331_hlt_ili9883c");
}

static void lcm_init_power(void)
{
	pr_info("ili9883c_boe lcm_init_power\n");
	MDELAY(1);
	SET_LCM_VSP_PIN(1);
	MDELAY(3);
	SET_LCM_VSN_PIN(1);
...
static void lcm_suspend_power(void)
{
	if((0 == tp_gesture_enable_flag()) || esd_flag == 1) {
		pr_info("ili9883c_boe lcm_suspend_power\n");
		SET_LCM_VSN_PIN(0);
		MDELAY(2);
		SET_LCM_VSP_PIN(0);
	}
...
static void lcm_setbacklight_cmdq(void *handle, unsigned int level)
{
	...
	pr_info("[HW check backlight ili9883c_boe]level=%d,para_list[0]=%x,para_list[1]=%x\n", level, bl_level[1].para_list[0], bl_level[1].para_list[1]);
...
static unsigned int lcm_esd_recover(void)
{
	...
	if (lcm_dsi_mode == CMD_MODE) {
		push_table(NULL, init_setting_cmd, sizeof(init_setting_cmd) / sizeof(struct LCM_setting_table), 1);
		pr_info("ili9883c_boe = cmd mode esd recovery :%d----\n", lcm_dsi_mode);
	} else {
		push_table(NULL, init_setting_vdo, sizeof(init_setting_vdo) / sizeof(struct LCM_setting_table), 1);
		push_table(NULL, lcm_display_on, sizeof(lcm_display_on) / sizeof(struct LCM_setting_table), 1);
		pr_info("ili9883c_boe = vdo mode esd recovery :%d----\n", lcm_dsi_mode);
	}
...
struct LCM_DRIVER oplus21331_ili9883c_hlt_hdp_dsi_vdo_lcm_drv =
{
	.name = "oplus21331_ili9883c_hlt_hdp_dsi_vdo_lcm",

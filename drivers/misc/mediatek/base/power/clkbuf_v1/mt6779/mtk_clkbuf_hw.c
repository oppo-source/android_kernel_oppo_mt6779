...
static int clkbuf_read_pmic(u32 addr, u32 *val, u32 mask, u32 shift)
{
	int ret;

	if (!clkbuf_regmap) {
		pr_notice("error: clkbuf_regmap = %x\n", clkbuf_regmap);
		return -1;
	}

	ret = regmap_read(clkbuf_regmap, addr, val);
	*val = (*val >> shift) & mask;

	return ret;
}

static int clkbuf_write_pmic(u32 addr, u32 val, u32 mask, u32 shift)
{
	int ret;
	...
	ret = regmap_update_bits(clkbuf_regmap, addr, mask, val);
...
{
	if (on)
		clkbuf_write_pmic(PMIC_DCXO_CW09_SET_ADDR, 0x1,
				      PMIC_XO_EXTBUF7_EN_M_MASK,
				      PMIC_XO_EXTBUF7_EN_M_SHIFT);
	else
		clkbuf_write_pmic(PMIC_DCXO_CW09_CLR_ADDR, 0x1,
				      PMIC_XO_EXTBUF7_EN_M_MASK,
				      PMIC_XO_EXTBUF7_EN_M_SHIFT);
}
...
void clk_buf_ctrl_bblpm_hw(short on)
{
	...
	if (!is_pmic_clkbuf)
		return;
	if (on) {
		clkbuf_write_pmic(PMIC_XO_BB_LPM_EN_SEL_ADDR, 0x1,
				      PMIC_XO_BB_LPM_EN_SEL_MASK,
				      PMIC_XO_BB_LPM_EN_SEL_SHIFT);
	} else {
		clkbuf_write_pmic(PMIC_XO_BB_LPM_EN_SEL_ADDR, 0x0,
				      PMIC_XO_BB_LPM_EN_SEL_MASK,
				      PMIC_XO_BB_LPM_EN_SEL_SHIFT);
	}

	clkbuf_read_pmic(PMIC_XO_BB_LPM_EN_SEL_ADDR, &bblpm_sel,
...
void clk_buf_control_bblpm(bool on)
{
	...
	if (on) /* FPM -> BBLPM */
		clkbuf_write_pmic(PMIC_DCXO_CW00_SET_ADDR, 0x1,
				      PMIC_XO_BB_LPM_EN_M_MASK,
				      PMIC_XO_BB_LPM_EN_M_SHIFT);
	else /* BBLPM -> FPM */
		clkbuf_write_pmic(PMIC_DCXO_CW00_CLR_ADDR, 0x1,
				      PMIC_XO_BB_LPM_EN_M_MASK,
				      PMIC_XO_BB_LPM_EN_M_SHIFT);

	clkbuf_read_pmic(PMIC_DCXO_CW00, &cw00,
			    PMIC_REG_MASK, PMIC_REG_SHIFT);
...
static void clk_buf_ctrl_internal(enum clk_buf_id id, enum clk_buf_onff onoff)
{
	...
	case CLK_BUF_CONN:
		if (onoff == CLK_BUF_FORCE_ON) {
			...
			clkbuf_write_pmic(PMIC_DCXO_CW00_CLR,
				PMIC_XO_EXTBUF2_MODE_MASK,
				PMIC_XO_EXTBUF2_MODE_MASK,
				PMIC_XO_EXTBUF2_MODE_SHIFT);
			clkbuf_write_pmic(PMIC_DCXO_CW00_SET,
			...
			clkbuf_write_pmic(PMIC_XO_EXTBUF2_MODE_ADDR,
				xo_mode_init[XO_WCN],
				PMIC_XO_EXTBUF2_MODE_MASK,
				PMIC_XO_EXTBUF2_MODE_SHIFT);
			pwrap_dcxo_en =
	...
	case CLK_BUF_NFC:
		if (onoff == CLK_BUF_FORCE_ON) {
			...
			clkbuf_write_pmic(PMIC_XO_EXTBUF3_MODE_ADDR,
				xo_mode_init[XO_NFC],
				PMIC_XO_EXTBUF3_MODE_MASK,
				PMIC_XO_EXTBUF3_MODE_SHIFT);
			pwrap_dcxo_en =
	...
	case CLK_BUF_RF:
		if (onoff == CLK_BUF_FORCE_ON) {
			...
			clkbuf_write_pmic(PMIC_XO_EXTBUF4_MODE_ADDR,
				xo_mode_init[XO_CEL],
				PMIC_XO_EXTBUF4_MODE_MASK,
				PMIC_XO_EXTBUF4_MODE_SHIFT);
		}
	...
	case CLK_BUF_UFS:
		if (onoff == CLK_BUF_FORCE_ON) {
			clk_buf7_ctrl = false;
			clkbuf_write_pmic(PMIC_DCXO_CW09_CLR,
				      PMIC_XO_EXTBUF7_MODE_MASK,
				      PMIC_XO_EXTBUF7_MODE_MASK,
				      PMIC_XO_EXTBUF7_MODE_SHIFT);
			clkbuf_write_pmic(PMIC_DCXO_CW09_SET,
				PMIC_XO_EXTBUF7_EN_M_MASK,
				PMIC_XO_EXTBUF7_EN_M_MASK,
				PMIC_XO_EXTBUF7_EN_M_SHIFT);
			...
			clkbuf_write_pmic(PMIC_XO_EXTBUF7_MODE_ADDR,
				xo_mode_init[XO_EXT],
				PMIC_XO_EXTBUF7_MODE_MASK,
				PMIC_XO_EXTBUF7_MODE_SHIFT);
			clk_buf7_ctrl = true;
...
static void pmic_clk_buf_ctrl(enum CLK_BUF_SWCTRL_STATUS_T *status)
{
	...
	clkbuf_read_pmic(PMIC_DCXO_CW00, &pmic_cw00,
		PMIC_REG_MASK, PMIC_REG_SHIFT);
	clkbuf_read_pmic(PMIC_DCXO_CW09, &pmic_cw09,
		PMIC_REG_MASK, PMIC_REG_SHIFT);
...
static int clk_buf_mode_set(enum clk_buf_id id)
{
	...
	case CLK_BUF_BB_MD:
		break;
	case CLK_BUF_CONN:
		clkbuf_read_pmic(PMIC_XO_EXTBUF2_MODE_ADDR,
			&val,
			PMIC_XO_EXTBUF2_MODE_MASK,
			PMIC_XO_EXTBUF2_MODE_SHIFT);
		...
		break;
	case CLK_BUF_NFC:
		clkbuf_read_pmic(PMIC_XO_EXTBUF3_MODE_ADDR,
				&val,
				PMIC_XO_EXTBUF3_MODE_MASK,
				PMIC_XO_EXTBUF3_MODE_SHIFT);
		ret = val - BUF_MAN_M;
		break;
	case CLK_BUF_RF:
		clkbuf_read_pmic(PMIC_XO_EXTBUF4_MODE_ADDR,
				&val,
				PMIC_XO_EXTBUF4_MODE_MASK,
				PMIC_XO_EXTBUF4_MODE_SHIFT);
		ret = val - SIG_CTRL_M;
		break;
	case CLK_BUF_UFS:
		clkbuf_read_pmic(PMIC_XO_EXTBUF7_MODE_ADDR,
				&val,
				PMIC_XO_EXTBUF7_MODE_MASK,
				PMIC_XO_EXTBUF7_MODE_SHIFT);
		ret = val - BUF_MAN_M;
...
void clk_buf_dump_clkbuf_log(void)
{
	...
	clkbuf_read_pmic(PMIC_XO_EXTBUF1_MODE_ADDR, &pmic_cw00,
		PMIC_REG_MASK, PMIC_REG_SHIFT);
	clkbuf_read_pmic(PMIC_XO_EXTBUF7_MODE_ADDR, &pmic_cw09,
		PMIC_REG_MASK, PMIC_REG_SHIFT);
	clkbuf_read_pmic(PMIC_XO_EXTBUF2_CLKSEL_MAN_ADDR, &pmic_cw12,
		PMIC_REG_MASK, PMIC_REG_SHIFT);
	clkbuf_read_pmic(PMIC_RG_XO_EXTBUF2_SRSEL_ADDR, &pmic_cw13,
		PMIC_REG_MASK, PMIC_REG_SHIFT);
	clkbuf_read_pmic(PMIC_RG_XO_RESERVED1_ADDR, &pmic_cw15,
		PMIC_REG_MASK, PMIC_REG_SHIFT);
	clkbuf_read_pmic(PMIC_RG_XO_EXTBUF2_RSEL_ADDR, &pmic_cw19,
		PMIC_REG_MASK, PMIC_REG_SHIFT);
	clkbuf_read_pmic(PMIC_RG_SRCLKEN_IN3_EN_ADDR, &top_spi_con1,
		PMIC_RG_SRCLKEN_IN3_EN_MASK, PMIC_RG_SRCLKEN_IN3_EN_SHIFT);
	clkbuf_read_pmic(PMIC_RG_LDO_VRFCK_HW14_OP_EN_ADDR, &ldo_vrfck_op,
		PMIC_RG_LDO_VRFCK_HW14_OP_EN_MASK,
		PMIC_RG_LDO_VRFCK_HW14_OP_EN_SHIFT);
	clkbuf_read_pmic(PMIC_RG_LDO_VBBCK_HW14_OP_EN_ADDR, &ldo_vbbck_op,
		PMIC_RG_LDO_VBBCK_HW14_OP_EN_MASK,
		PMIC_RG_LDO_VBBCK_HW14_OP_EN_SHIFT);
	clkbuf_read_pmic(PMIC_RG_LDO_VRFCK_EN_ADDR, &ldo_vrfck_en,
		PMIC_RG_LDO_VRFCK_EN_MASK, PMIC_RG_LDO_VRFCK_EN_SHIFT);
	clkbuf_read_pmic(PMIC_RG_LDO_VBBCK_EN_ADDR, &ldo_vbbck_en,
...
static u32 dcxo_dbg_read_auxout(u16 sel)
{
	...
	clkbuf_write_pmic(PMIC_XO_STATIC_AUXOUT_SEL_ADDR, sel,
			      PMIC_XO_STATIC_AUXOUT_SEL_MASK,
			      PMIC_XO_STATIC_AUXOUT_SEL_SHIFT);
	clkbuf_read_pmic(PMIC_XO_STATIC_AUXOUT_ADDR, &rg_auxout,
...
static ssize_t clk_buf_show_status_info_internal(char *buf)
{
	...
	clkbuf_read_pmic(PMIC_DCXO_CW09, &pmic_cw09,
			    PMIC_REG_MASK, PMIC_REG_SHIFT);
	clkbuf_read_pmic(PMIC_DCXO_CW12, &pmic_cw12,
			    PMIC_REG_MASK, PMIC_REG_SHIFT);
	clkbuf_read_pmic(PMIC_DCXO_CW13, &pmic_cw13,
			    PMIC_REG_MASK, PMIC_REG_SHIFT);
	clkbuf_read_pmic(PMIC_DCXO_CW15, &pmic_cw15,
			    PMIC_REG_MASK, PMIC_REG_SHIFT);
	clkbuf_read_pmic(PMIC_DCXO_CW19, &pmic_cw19,
			    PMIC_REG_MASK, PMIC_REG_SHIFT);
	clkbuf_read_pmic(PMIC_RG_LDO_VRFCK_HW14_OP_EN_ADDR,
			    &ldo_vrfck_op_en,
			    PMIC_RG_LDO_VRFCK_HW14_OP_EN_MASK,
			    PMIC_RG_LDO_VRFCK_HW14_OP_EN_SHIFT);
	clkbuf_read_pmic(PMIC_RG_LDO_VBBCK_HW14_OP_EN_ADDR,
			    &ldo_vbbck_op_en,
			    PMIC_RG_LDO_VBBCK_HW14_OP_EN_MASK,
			    PMIC_RG_LDO_VBBCK_HW14_OP_EN_SHIFT);
	clkbuf_read_pmic(PMIC_RG_LDO_VRFCK_EN_ADDR, &ldo_vrfck_en,

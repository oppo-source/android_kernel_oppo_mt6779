...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
    ...
    while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		data_index = 0;
		read_eepromData(&EepData[data_index], GC02M0F_STEREO_START_ADDR, DUALCAM_CALI_DATA_LENGTH, SLAVEADDR);
		data_index += DUALCAM_CALI_DATA_LENGTH;
		/*Readout SN data*/
		read_eepromData(&EepData[data_index], 0x40, CAMERA_MODULE_SN_LENGTH, SLAVEADDR);

...
void oplus_tempntc_read_ntcswitch1_high(struct temp_data *info) {
	...
	if (ret < 0) {
		pr_err("%s CHARGE_NTC read error!\n", __func__);
	} else {
...
void oplus_tempntc_read_ntcswitch2_high(struct temp_data *info) {
	...
	if (ret < 0) {
		pr_err("%s BAT_CON_NTC read error!\n", __func__);
	} else {
...
int oplus_tempntc_get_volt(struct temp_data *info)
{
	...
	printk("%s ntcswitch1_gpio_value = %d , ntcswitch2_gpio_value = %d.\n",
		 __func__, ntcswitch1_gpio_value, ntcswitch2_gpio_value);
	...
	if (ntcswitch1_gpio_value == 0) {
		...
		if (ret < 0) {
			pr_err("%s ntcswitch1_gpio_value = %d\n", ret);
		}
	...
	if (ntcswitch2_gpio_value == 0) {
		...
		if (ret < 0) {
			pr_err("%s ntcswitch2_gpio_value = %d\n", ret);
		}
	...
	pr_err("%s BAT_CON_NTC = %d, PA_NTC = %d, CHARGE_NTC = %d\
		BB_NTC = %d, FLASH_NTC = %d, BAT_ID = %d.\n",
		__func__, info->bat_con_ntc_volt, info->pa_ntc_volt, info->charger_ntc_volt,
...
static int oplus_ntcctrl_gpio_init(struct temp_data *info,struct device *dev)
{
	...
	if (IS_ERR_OR_NULL(info->ntc_switch1_ctrl_high)) {
		printk("get ntc_switch1_ctrl_high fail\n");
		return -EINVAL;
	}
	...
	if (IS_ERR_OR_NULL(info->ntc_switch1_ctrl_low)) {
		printk("get ntc_switch1_ctrl_low fail\n");
		return -EINVAL;
	}
	...
	if (IS_ERR_OR_NULL(info->ntc_switch2_ctrl_high)) {
		printk("get ntc_switch2_ctrl_high fail\n");
		return -EINVAL;
	}
	...
	if (IS_ERR_OR_NULL(info->ntc_switch2_ctrl_low)) {
		printk("get ntc_switch2_ctrl_low fail\n");
		return -EINVAL;
	}
